          ...
          onPressed: locationProvider.loading ? null : () async {
            ...
            if(!isAvailable) {
              ...
              if (isEnableUpdate) {
                ...
                locationProvider.addAddress(addressModel, context).then((value) async {

                  if (value.isSuccess) {
                    if (fromCheckout) {
                      final orderProvider = Provider.of<OrderProvider>(Get.context!, listen: false);
                      final locationProviderContext = Provider.of<LocationProvider>(Get.context!, listen: false);
                      final splashProvider = Provider.of<SplashProvider>(Get.context!, listen: false);

                      final oldListLength = locationProviderContext.addressList?.length ?? 0;
                      await locationProviderContext.initAddressList();
                      final addressList = locationProviderContext.addressList;
                      if (addressList != null && addressList.isNotEmpty) {
                        ...
                        if (oldListLength == 0) {
                          ...
                          newAddressIndex = addressList.length - 1;
                        }

                        final newAddress = addressList[newAddressIndex];
                        ...
                        if (isDistanceWiseDelivery) {
                          if ((newAddress.latitude == null || newAddress.latitude!.isEmpty) ||
                              (newAddress.longitude == null || newAddress.longitude!.isEmpty)) {
                            isAvailable = false;
                          }
                        }
                        ...
                        if (isAvailable) {
                          await CheckOutHelper.selectDeliveryAddress(
                            ...
                            locationProvider: locationProviderContext,
