...
class ProfileRepo {
  ...
  Future<ApiResponseModel> getAddressTypeList() async {
    try {
      List<String> addressTypeList = ['Select Address type', 'Home', 'Office', 'Other'];
  ...
  Future<ApiResponseModel> getMemberStatus() async {
    try {
      final response = await dioClient!.get(AppConstants.customerMemberStatusUri);
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<http.StreamedResponse> updateProfile(UserInfoModel userInfoModel, String pass, File? file, PickedFile? data, String token) async {
    ...
    fields.addAll(<String, String>{'_method': 'put', 'f_name': userInfoModel.fName!, 'l_name': userInfoModel.lName!, 'email': userInfoModel.email!, 'phone': userInfoModel.phone!, 'password': pass});
