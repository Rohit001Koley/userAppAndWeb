    ...
    return CustomPopScopeHandelDeepLinkWidget(
      child: Scaffold(
        appBar: (ResponsiveHelper.isDesktop(context) ? const PreferredSize(preferredSize: Size.fromHeight(120), child: WebAppBarWidget())
            : CustomAppBarWidget(title:  getTranslated(widget.productType, context),fromHomeItem: true, productType : widget.productType))
        as PreferredSizeWidget?,
        body: Consumer<CategoryProvider>(
          builder: (context, productProvider, child) {
            return Center(child: CustomScrollView(controller: scrollController, slivers: [
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.10), offset: const Offset(0, 4), blurRadius: 7, spreadRadius: 0.1)],
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('${getTranslated('sort_by', context)} : ', style: poppinsRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                            Padding(
                              padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
                              child: categoryProvider.allSortBy.isNotEmpty ?
                              PopupMenuButton(
                                elevation: 20,
                                offset: const Offset(20, 28),
                                initialValue: categoryProvider.sortByIndex >= 0 ? categoryProvider.allSortBy[
                                categoryProvider.sortByIndex.clamp(0, categoryProvider.allSortBy.length - 1)] : null,
                                child: Row(children: [
                                  Text(categoryProvider.sortByIndex >= 0 ? getTranslated(categoryProvider.allSortBy[categoryProvider.sortByIndex.clamp(0, categoryProvider.allSortBy.length - 1)] ?? '', context) : 'default'.tr,
                                ...
                                onSelected: (dynamic value) {
                                  ...
                                  if(index >= 0) {
                                    ...
                                    Provider.of<ProductProvider>(context, listen: false).getItemList(1, productType: widget.productType, sortBy: value);
                                  }
                                },
                                itemBuilder: (context) {
                                  ...
                                  : Text('-', style: poppinsRegular.copyWith(fontSize: Dimensions.fontSizeDefault)),
                            ),
                          ],
                        ),
                      ),
                          ...
                          return productModel == null ? CustomLoaderWidget(
                            ...
                            onPaginate: (int? offset) async {
                              if(widget.productType == ProductType.flashSale) {
                                ...
                                final String? sortBy = (categoryProvider.allSortBy.isNotEmpty)
                                    ? categoryProvider.allSortBy[
                                categoryProvider.sortByIndex.clamp(0, categoryProvider.allSortBy.length - 1)]
                                    : null;
                                await productProvider.getItemList(offset ?? 1, productType:  widget.productType, sortBy: sortBy);
