...
import 'package:flutter_grocery/common/widgets/indicator_circle_widget.dart';
...
import 'package:flutter_grocery/common/widgets/search_filter_widget.dart';
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Row(
                              children: [
                                Text("${searchProvider.searchProductModel?.totalSize ?? 0} ", style: poppinsMedium,),
                                Text(getTranslated('items_found', context), style: poppinsMedium,),
                              ],
                            ),
                            searchProvider.searchProductModel?.products != null ? Builder(
                              builder: (context) {
                                final List<dynamic> categories = Provider.of<CategoryProvider>(context, listen: false).categoryList ?? [];
                                  ...
                                  children: [
                                    ...
                                    if (searchProvider.hasActiveFilters)
                                      Positioned(
                                        right: 10,
                                        top: 5,
                                        child: const ActiveStatusIndicatorWidget(),
                                      ),
                                  ],
                                );
                      ...
                      searchProvider.searchProductModel?.products == null ? GridView.builder(
                        ...
                        onPaginate: (offset) async => await searchProvider.getSearchProduct(
                          offset: offset ?? 1,
                          query: (widget.searchString ?? '').trim().isEmpty ? 'a' : widget.searchString ?? '',
                          priceLow: searchProvider.lowerValue,
                          priceHigh: searchProvider.upperValue,
                          filterType: searchProvider.selectedFilter,
                          rating: searchProvider.selectedRatingIndex,
                          categories: searchProvider.selectedCategories.isEmpty ? null : searchProvider.selectedCategories,
                        ),
