...
class PaymentSectionWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
      ...
      bool showAddButtonForWarning = orderProvider.partialAmount != null && orderProvider.partialAmount! > 0 && 
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Padding(
              ...
              child: TextButton(
                ...
                child: Text(getTranslated(showPayment && !showAddButtonForWarning ? 'change' : 'add', context), style: poppinsBold.copyWith(
...
class _SelectedPaymentView extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final OrderProvider checkoutProvider = Provider.of<OrderProvider>(context, listen: true);
    ...
    return  Column(children: [
      if(checkoutProvider.partialAmount == null) rowTextWidgetAnimated(
      ...
      if(checkoutProvider.partialAmount != null) ...[
        ...
        if(checkoutProvider.paymentMethod != null || checkoutProvider.selectedOfflineMethod != null || checkoutProvider.paymentMethodIndex == 1)
          rowTextWidgetAnimated(
              title: '${checkoutProvider.selectedOfflineMethod != null
                  ? checkoutProvider.selectedOfflineMethod?.methodName
                  : (checkoutProvider.paymentMethod?.getWayTitle ?? getTranslated('cash_on_delivery', context))} (${getTranslated('due', context)})',
          ...
          Container(
            ...
            child: Row(children: [
              ...
              const SizedBox(width: Dimensions.paddingSizeSmall),
              Expanded(child: Text(
                getTranslated('please_select_another_payment_method_for_remaining_amount', context),
  ...
  Widget rowTextWidgetAnimated({required String title, required double price, required BuildContext context}){
    return  Row(children: [
      Expanded(child: Text(title,
        style: poppinsMedium.copyWith(fontSize: Dimensions.fontSizeDefault,
          color: Theme.of(context).textTheme.bodySmall?.color?.withValues(alpha: 0.7),
      ...
      Directionality(
        ...
        child: PriceConverterHelper.convertAnimationPrice(
          context, 
          price,
          textStyle: poppinsSemiBold.copyWith(fontSize: Dimensions.fontSizeDefault),
        ),
      ),
    ]);
  }
