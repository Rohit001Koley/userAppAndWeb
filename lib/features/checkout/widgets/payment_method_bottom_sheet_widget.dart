...
class _PaymentMethodBottomSheetWidgetState extends State<PaymentMethodBottomSheetWidget> {
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    borderRadius: ResponsiveHelper.isMobile()
                        ? const BorderRadius.vertical(top: Radius.circular(Dimensions.radiusSizeLarge))
                  ...
                  child: SafeArea(
                    child: Consumer<OrderProvider>(
                      builder: (ctx, orderProvider, _) {
                        ...
                        bool hideCOD = isWalletSelectAndNotPartial || (_partialAmount != null && (partialPaymentCombinator == "digital_payment" || partialPaymentCombinator == "offline_payment"));

                        bool hideDigital = isWalletSelectAndNotPartial || (_partialAmount != null && (partialPaymentCombinator == "cod" || partialPaymentCombinator == "offline_payment"));

                        bool hideOffline = isWalletSelectAndNotPartial || (_partialAmount != null && (partialPaymentCombinator == "cod" || partialPaymentCombinator == "digital_payment"));
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Align(
                                ...
                                child: InkWell(onTap: () => Navigator.of(context).pop(), child: const Icon(Icons.clear, size: 20)),
                              ),
                            ),
                            ...
                            Text(
                              ...
                              style: poppinsRegular.copyWith(fontSize: Dimensions.fontSizeSmall, color: Theme.of(context).textTheme.bodySmall?.color?.withValues(alpha: 0.7)),
                            ),
                            PriceConverterHelper.convertAnimationPrice(context, widget.totalPrice, textStyle: poppinsBold.copyWith(fontSize: Dimensions.fontSizeLarge)),

                            const SizedBox(height: Dimensions.paddingSizeDefault),

                            ConstrainedBox(
                              constraints: BoxConstraints(minHeight: MediaQuery.of(context).size.height * 0.2, maxHeight: MediaQuery.of(context).size.height * 0.5),
                              child: SingleChildScrollView(
                                child: Padding(
                                  padding: EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeLarge * (ResponsiveHelper.isDesktop(context) ? 2 : 1)),
                                  child: Column(
                                    children: [
                                      configModel.walletStatus! && authProvider.isLoggedIn() && walletBalance > 0 && !widget.isAlreadyPartialApplied
                                          ? PaymentButtonWidget(
                                      ...
                                      if (configModel.cashOnDelivery!) BringChangeInputWidget(amountController: _bringAmountController, hidePaymentMethod: hideCOD),
                                        ...
                                        Opacity(
                                          ...
                                          child: Stack(
                                            children: [
                                              ...
                                              if (hideOffline && hideDigital) Positioned.fill(child: Container(color: Colors.transparent)),
                                            ],
                                          ),
                                        ),
                            ...
                            Padding(
                              ...
                              child: orderProvider.isLoading
                                  ...
                                  : CustomButtonWidget(
                                      ...
                                      onPressed:
                                          ((_paymentMethodIndex == null && _paymentMethod == null) ||
                                              (_paymentMethod != null && _paymentMethod?.type == 'offline' && _selectedOfflineMethod == null) ||
                                              (_paymentMethodIndex == 0 && isPartialPayment && _paymentMethod == null && (_paymentMethodIndex == null || _paymentMethodIndex == 0)))
                                          ? null
                                          : () {
                                              if (_paymentMethod?.type == 'offline' && _selectedOfflineValue == null ||
                                                  (widget.orderId != null && widget.orderId != "null" && orderProvider.selectedOfflineValue != null)) {
                                                ...
                                                if (widget.orderId != null && widget.orderId != "null") {
                                                  String paymentMethod = orderProvider.selectedOfflineValue != null ? 'offline_payment' : orderProvider.selectedPaymentMethod!.getWay!;
  ...
  void initState() {
    ...
    if (configModel.isOfflinePayment!) {
      paymentList.add(PaymentMethod(getWay: 'offline', getWayTitle: getTranslated('offline', context), type: 'offline', getWayImage: Images.offlinePayment));
    }
    _initializeData();
  }
  ...
  void _callback(bool isSuccess, String message, String orderID) async {
    if (isSuccess) {
      ...
      showFlutterCustomToaster("now_pay_you_bill_using_the_payment_method", toasterTitle: "your_order_has_placed_successfully", context: context, type: ToasterMessageType.success, duration: 4);
    } else {
  ...
  Future<void> _switchOfflineToDigital({required String paymentMethod, required double orderAmount, double? partialAmount, required String orderId}) async {
    ...
    PlaceOrderModel placeOrderBody = PlaceOrderModel(
      ...
      callBack: ResponsiveHelper.isWeb() ? '$protocol//$hostname${kDebugMode ? ':$port' : ''}${RouteHelper.orderDetails}' : '${AppConstants.baseUrl}${RouteHelper.orderDetails}',
      orderId: orderId,
      shwitchToDigital: '1',
    );
    ...
    if (paymentUrl != null) {
      if (ResponsiveHelper.isWeb()) {
        ...
        RouteHelper.getPaymentRoute(url: paymentUrl, id: orderId, action: RouteAction.pushNamedAndRemoveUntil);
      }
    }
  }
}
