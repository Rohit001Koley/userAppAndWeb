        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Padding(
            ...
            child: Column(
              children: [
                Text(
                  widget.notification?.body ?? '', textAlign: TextAlign.center,
                  style: poppinsRegular.copyWith(fontSize: Dimensions.fontSizeLarge),
                ),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
           ...
           if(widget.notification?.orderId != null || widget.notification?.type == 'message' || widget.notification?.type == 'wallet') Flexible(
             child: SizedBox(
                width: 120,
                height: 40,
                    ...
                    try{
                      if(widget.notification?.orderId == null) {
                        RouteHelper.getChatRoute(
                          orderId: widget.notification!.orderId.toString(),
                          userName: widget.notification?.userName ?? "",
                          profileImage: widget.notification?.userImage ?? "",
                          senderType: widget.notification?.senderType ?? "admin"
                        );
                      }else if(widget.notification?.type == 'wallet'){
                        RouteHelper.getWalletRoute();
                      }else if(widget.notification?.orderId != null && widget.notification?.type == 'message'){
                        await orderProvider.trackOrder(widget.notification?.orderId.toString(), null, context, false, isUpdate: false);
                        RouteHelper.getChatRoute(
                            orderId: widget.notification!.orderId.toString(),
                            userName: widget.notification?.userName ?? "",
                            profileImage: widget.notification?.userImage ?? "",
                        ...
                        RouteHelper.getOrderDetailsRoute(widget.notification?.orderId.toString());
                      }
