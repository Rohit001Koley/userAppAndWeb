...
class ProductProvider extends ChangeNotifier {
  ...
  Future<void> getItemList(int offset, {bool isUpdate = true, bool isReload = true,  required String? productType, ProductFilterType? filterType, String? sortBy}) async {
    if(offset == 1) {
    ...
    if(offset == 1) {
      DataSyncHelper.fetchAndSyncData(
        fetchFromLocal: ()=> productRepo.getItemList(offset, productType, source: DataSourceEnum.local, filterType: filterType, sortBy: sortBy),
        fetchFromClient: ()=> productRepo.getItemList(offset, productType, source: DataSourceEnum.client, filterType: filterType, sortBy: sortBy),
        onResponse: (data, _){
          if(productType == ProductType.dailyItem) {
      ...
      ApiResponseModel apiResponse = await productRepo.getItemList(offset, productType, source: DataSourceEnum.client, filterType: filterType, sortBy: sortBy);
