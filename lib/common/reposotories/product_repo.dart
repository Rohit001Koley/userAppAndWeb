...
class ProductRepo extends DataSyncRepo {
  ...
  Future<ApiResponseModel<T>> getItemList<T>(int offset, String? productType, {required DataSourceEnum source, ProductFilterType? filterType, String? sortBy}) async {
    String? apiUrl;
  ...
  Future<ApiResponseModel> getBrandOrCategoryProductList(
    ...
    try {
      final Map<String, dynamic> params = {
        'limit': 10,
        'offset': offset,
        if (priceLow != null && priceHigh != null) ...{'min_price': priceLow, 'max_price': priceHigh},
        'sort_by': ?sortBy,
        'rating': ?rating,
      };

      final response = await dioClient.get('${AppConstants.categoryProductUri}$id', queryParameters: params);
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
