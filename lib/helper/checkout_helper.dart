...
class CheckOutHelper {
  ...
  static void _revalidateWalletPaymentAfterChange(OrderProvider orderProvider) {
    ...
    bool hasSecondaryPayment = orderProvider.paymentMethod != null ||
        orderProvider.selectedOfflineMethod != null ||
        orderProvider.paymentMethodIndex == 1;
    if (authProvider.isLoggedIn() && orderProvider.paymentMethodIndex == 0 && !hasSecondaryPayment) {
      double walletBalance = profileProvider.userInfoModel?.walletBalance ?? 0.0;
      double totalAmount = (orderProvider.getCheckOutData?.amount ?? 0) + 
  ...
  static double weightChargeCalculation(double? weight, DeliveryInfoModel? deliveryInfoModel) {
    ...
    if (deliveryInfoModel?.deliveryWeightSettingsStatus ?? false) {
      ...
      if (deliveryInfoModel?.deliveryWeightChargeType == WeightChargeType.unit.name && weight != null && deliveryInfoModel?.deliveryCountChargeFrom != null) {
        ...
        if (deliveryInfoModel?.deliveryCountChargeFromOperation == WeightChargeConditions.greaterOrEqual.name &&
        ...
        for (var range in weightRanges) {
          ...
          } else if (minOperation == WeightChargeConditions.greaterOrEqual.name && weight! >= minWeight) {
            minConditionMet = true;
          }
          ...
          } else if (maxOperation == WeightChargeConditions.lessOrEqual.name && weight! <= maxWeight) {
  ...
  static void autoSelectPaymentMethod({required double totalAmount}) {
    ...
    if(orderProvider.paymentMethodIndex == null && orderProvider.paymentMethod == null) {
      ...
      bool hasWallet = configModel.walletStatus! && authProvider.isLoggedIn() && walletBalance > 0;
      ...
      int digitalPaymentCount = (configModel.activePaymentMethodList?.length ?? 0);
      ...
      if(configModel.isOfflinePayment!){
        int offlineMethodCount = splashProvider.offlinePaymentModelList?.length ?? 0;
        if(offlineMethodCount == 1){
          totalPaymentOptions++;
          autoSelectMethod = PaymentMethod(
            getWay: 'offline', getWayTitle: 'Offline',
            type: 'offline',
          );
